    ...
    $('#requestapp form').on('submit', function (e){
        ...
        $.post(window.wp.ajax_url, data, function(res){
            if (res.success){
                ...
                $('#requestapp').html('<div class="alert alert-success" role="alert">' +
                    'Заявка успешно добавлена. Руководителю установки и вам направлено письмо, содержащее эту заявку, ' +
                    'дальнейший статус заявки узнавайте на специальной страничке. ' +
                    'Если заявка вам не пришла, попробуйте поискать в папке со спамом.</div>');
            } else {
                alert(res.err);
            }
        }, 'json');
